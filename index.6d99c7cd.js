...
const $56a0b18e519895ee$var$setState = (item)=>{
    if (item) {
        if (item.some((element)=>Array.isArray(element))) for (const element of item){
...
const $56a0b18e519895ee$var$handleMapContextmenu = (event)=>{
    ...
    const target = event.target;
    if ($56a0b18e519895ee$var$isInvalidElement(target)) return;
    const siblings = Array.from(target.parentNode.childNodes).filter((element)=>element.tagName === "path" && !element.classList.contains("landxx"));
    if (target.style.fill) {
        for (const item of siblings){
            item.style.fill = "";
            delete $56a0b18e519895ee$var$state[item.id];
        }
        $56a0b18e519895ee$var$setState();
    } else {
        const color = $56a0b18e519895ee$var$colorPicker.value;
        ...
        $56a0b18e519895ee$var$setState(paths);
    }
};
...
//# sourceMappingURL=index.6d99c7cd.js.map
